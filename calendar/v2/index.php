...
$APPLICATION->SetTitle("Фотоотчеты по витринам V2");

//Список отделов
$arDepartament = hl_getlist(HLBL_PHOTOSSHOWCASEDEPARTAMENTS, [], 'ID', ['UF_NAME' => 'ASC']);
$arShopsList = hl_getlist(HLBL_SHOPS, ['=UF_STATUS' => 1], 'UF_XML_ID', ['UF_NAME' => 'ASC']);
$_monthsList = [
    "01" => "январь",
    "02" => "февраль",
    "03" => "март",
...
$currentWeek = strtotime(current($arInterval));
...
<script>
  ...
  function getCurrentWeek () {
    ...
    $.ajax({
      ...
      $.each(answer.SUMM, function (indexShop, count) {
        if (count>=5) {
          count = `<span class="badge badge-success">${count}</span>`
        } else if ((count<5) && (count>0)) {
  ...
  function getWeek (step, dataValue) {
    ...
    $.ajax({
      ...
      $('[data-num-header="1"]').text(answer.day_num[0])
      $('[data-num-header="2"]').text(answer.day_num[1])
      $('[data-num-header="3"]').text(answer.day_num[2])
      $('[data-num-header="4"]').text(answer.day_num[3])
      $('[data-num-header="5"]').text(answer.day_num[4])
      $('[data-num-header="6"]').text(answer.day_num[5])
      $('[data-num-header="7"]').text(answer.day_num[6])
